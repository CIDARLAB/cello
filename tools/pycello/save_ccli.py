...
@click.pass_context
def submit(ctx, jobid):

    endpoint = ctx.obj.url_root + "/submit"

    inputs_text = open("resources/Inputs.txt", 'r').read()
    outputs_text = open("resources/Outputs.txt", 'r').read()
    verilog_text = open("resources/0xFE.v", 'r').read()
    user_options = ""
...
def read_genbank(ctx, jobid, filename, seq):
    ...
    # output_handle = open(filepath.replace('.ape', '.fasta'), "w")
