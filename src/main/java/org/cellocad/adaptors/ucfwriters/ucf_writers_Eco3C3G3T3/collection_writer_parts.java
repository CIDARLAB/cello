...
public class collection_writer_parts extends collection_writer {
    ...
    public ArrayList<Map> getObjects() {
        ...
        ArrayList<ArrayList<String>> AND_part_tokens = Util.fileTokenizer(getRootPath() + "/resources/data/and_gates/AND_parts.txt");
        for(ArrayList<String> part: AND_part_tokens) {
            String name = part.get(0);
            String type = part.get(1);
            String seq  = part.get(2);

            Map p = new LinkedHashMap();
            p.put("collection", "parts"); //tag
            p.put("type", type);
            p.put("name", name);
            p.put("dnasequence", seq);
